...
function FranRegData() {
  ...
  const brandname = events.map(
    (item) => item.content.personal_details.brand_name
  );
  ...
  const PostDelete = (_id, e) => {
    ...
    axios
      .delete(
        `https://franchise-hub-server.herokuapp.com/api/v1/admin/dashboard/forms/franchisor-registration/${_id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
